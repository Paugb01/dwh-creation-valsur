...
class BatchExtractor:
    ...
    def __init__(self, config=None, secrets=None, config_path: str = "config/config.json", secrets_path: str = os.getenv("GOOGLE_APPLICATION_CREDENTIALS", "config/secrets.json")):
        # Load config either from dictionary or file
        if isinstance(config, dict):
            self.config = config
        else:
            self.config = self.load_config(config_path)
            
        # Load secrets either from dictionary or file  
    ...
    def upload_to_gcs(self, local_path: str, table_name: str, extraction_type: str = "batch") -> Optional[str]:
        ...
        try:
            ...
            # Create hive-friendly GCS path with date partitioning
            now = datetime.now()
            filename = Path(local_path).name
            database_name = self.secrets.get('mysql', {}).get('database', 'unknown_db')
            blob_path = f"bronze/{database_name}/{table_name}/year={now:%Y}/month={now:%m}/day={now:%d}/{filename}"
